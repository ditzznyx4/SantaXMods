...
-- ══ FAST RELOAD — infinite ammo feel: skip animasi reload, isi langsung ══
-- Cara kerja: percepat animasi reload 100x + langsung set ammo penuh via tool
local reloadConn
local function applyFastReload(active)
    ...
    if not active then
        -- Restore semua track reload ke speed normal
        for _,t in ipairs(a:GetPlayingAnimationTracks()) do
            if t.Name:lower():find("reload") then t:AdjustSpeed(1) end
        end
        return
    end

    -- Percepat reload yang sedang berjalan
    for _,t in ipairs(a:GetPlayingAnimationTracks()) do
        if t.Name:lower():find("reload") then t:AdjustSpeed(100) end
    end

    -- Hook animasi reload baru yang dimulai
    reloadConn = a.AnimationPlayed:Connect(function(track)
        if not State.FastReload then return end
        local n = track.Name:lower()
        if n:find("reload") then
            track:AdjustSpeed(100)
            -- Cari tool dengan ammo dan isi penuh langsung
            task.spawn(function()
                task.wait(0.01)
                local char2=getChar(LocalPlayer) if not char2 then return end
                -- Cari gun/tool di karakter
                for _,tool in ipairs(char2:GetChildren()) do
                    if tool:IsA("Tool") then
                        -- Arsenal gun: biasanya ada StringValue atau IntValue "Ammo"
                        local ammo = tool:FindFirstChild("Ammo")
                            or tool:FindFirstChild("ammo")
                            or tool:FindFirstChild("CurrentAmmo")
                        if ammo and (ammo:IsA("IntValue") or ammo:IsA("NumberValue")) then
                            ammo.Value = ammo.Value + 9999
                        end
                        -- Coba via ModuleScript/configurasi
                        local config = tool:FindFirstChild("Config")
                            or tool:FindFirstChild("Configuration")
                        if config then
                            local maxAmmo = config:FindFirstChild("MaxAmmo")
                                or config:FindFirstChild("Ammo")
                            if maxAmmo and maxAmmo:IsA("NumberValue") then
                                maxAmmo.Value = maxAmmo.Value + 9999
                            end
                        end
                    end
                end
...
-- ══ FAST FIRE — 50x fire speed nyata ══
-- Cara kerja:
-- 1. Percepat animasi fire 50x
-- 2. Manipulasi FireRate / Debounce di tool Arsenal langsung
-- 3. Spam RemoteEvent fire jika tool punya remote
local fireConn, fireSpamConn
local function applyFastFire(active)
    if fireConn then fireConn:Disconnect() fireConn=nil end
    if fireSpamConn then fireSpamConn:Disconnect() fireSpamConn=nil end
    ...
    if not active then
        for _,t in ipairs(a:GetPlayingAnimationTracks()) do
            if t.Name:lower():find("fire") or t.Name:lower():find("shoot") then
                t:AdjustSpeed(1)
            end
        end
        -- Restore fire rate tool
        for _,tool in ipairs(c:GetChildren()) do
            if tool:IsA("Tool") then
                local cfg = tool:FindFirstChild("Config") or tool:FindFirstChild("Configuration")
                if cfg then
                    local fr = cfg:FindFirstChild("FireRate") or cfg:FindFirstChild("Firerate")
                        or cfg:FindFirstChild("RPM") or cfg:FindFirstChild("Delay")
                    if fr and fr:IsA("NumberValue") then
                        -- Restore tidak bisa (nilai asli tidak disimpan), set ke nilai wajar
                        fr.Value = math.max(fr.Value, 0.08)
                    end
                end
                -- Debounce
                local db = tool:FindFirstChild("Debounce") or tool:FindFirstChild("debounce")
                if db and db:IsA("BoolValue") then db.Value = false end
            end
        end
        return
    end

    -- Percepat animasi fire yang sudah berjalan
    for _,t in ipairs(a:GetPlayingAnimationTracks()) do
        local n=t.Name:lower()
        if n:find("fire") or n:find("shoot") then t:AdjustSpeed(50) end
    end

    -- Hook animasi fire baru
    fireConn = a.AnimationPlayed:Connect(function(track)
        if not State.SpeedFire then return end
        local n=track.Name:lower()
        if n:find("fire") or n:find("shoot") then track:AdjustSpeed(50) end
    end)

    -- Heartbeat: terus manipulasi FireRate dan Debounce di tool aktif
    fireSpamConn = RunService.Heartbeat:Connect(function()
        if not State.SpeedFire then return end
        local char2=getChar(LocalPlayer) if not char2 then return end
        for _,tool in ipairs(char2:GetChildren()) do
            if tool:IsA("Tool") then
                -- Paksa FireRate jadi sangat kecil
                local cfg = tool:FindFirstChild("Config") or tool:FindFirstChild("Configuration")
                if cfg then
                    local fr = cfg:FindFirstChild("FireRate") or cfg:FindFirstChild("Firerate")
                        or cfg:FindFirstChild("RPM") or cfg:FindFirstChild("Delay")
                    if fr and fr:IsA("NumberValue") and fr.Value > 0.001 then
                        fr.Value = 0.001
                    end
                end
                -- Reset debounce setiap frame supaya tidak ada delay antar tembakan
                local db = tool:FindFirstChild("Debounce") or tool:FindFirstChild("debounce")
                if db and db:IsA("BoolValue") then db.Value = false end
                -- CoolDown
...
MainFrame.Size=UDim2.new(0,390,0,430)     -- lebih kecil dari sebelumnya
MainFrame.Position=UDim2.new(0.5,-195,0.5,-215)
...
-- ══ FLY TOGGLE ROW — checklist biasa + card box abu transparan outline merah di bawahnya ══
-- Struktur:
--   [checklist row biasa]
--   [card box: header text + toggle switch on/off]
local function flyToggleRow(labelText, key, order, cb)
    -- Wrapper container (checklist + card jadi satu blok)
    local wrapper=Instance.new("Frame",ContentFrame)
    wrapper.LayoutOrder=order
    wrapper.Size=UDim2.new(1,0,0,80)   -- checklist 25 + gap 4 + card 51
    wrapper.BackgroundTransparency=1 wrapper.BorderSizePixel=0

    -- ── Checklist row (sama persis seperti toggle biasa) ──
    local row=Instance.new("Frame",wrapper)
    row.Size=UDim2.new(1,0,0,25) row.Position=UDim2.new(0,0,0,0)
    row.BackgroundColor3=ROW row.BorderSizePixel=0
    Instance.new("UICorner",row).CornerRadius=UDim.new(0,4)
    ...
    lbl.Size=UDim2.new(1,-28,1,0) lbl.Position=UDim2.new(0,25,0,0)
    lbl.BackgroundTransparency=1 lbl.Font=Enum.Font.Gotham
    lbl.TextSize=11 lbl.TextColor3=WHITE lbl.TextXAlignment=Enum.TextXAlignment.Left
    lbl.Text=labelText

    -- ── Card box ──
    -- latar abu transparan, outline merah tipis, header text + toggle switch
    local card=Instance.new("Frame",wrapper)
    card.Size=UDim2.new(1,0,0,51) card.Position=UDim2.new(0,0,0,29)
    card.BackgroundColor3=Color3.fromRGB(55,55,55)
    card.BackgroundTransparency=0.45
    card.BorderSizePixel=0
    Instance.new("UICorner",card).CornerRadius=UDim.new(0,5)
    local cardStroke=Instance.new("UIStroke",card)
    cardStroke.Color=RED cardStroke.Thickness=1

    -- Header text (dalam card paling atas, dengan dasar hitam sedikit transparan)
    local headerBg=Instance.new("Frame",card)
    headerBg.Size=UDim2.new(1,0,0,20) headerBg.Position=UDim2.new(0,0,0,0)
    headerBg.BackgroundColor3=Color3.fromRGB(0,0,0)
    headerBg.BackgroundTransparency=0.4 headerBg.BorderSizePixel=0
    local hbCorner=Instance.new("UICorner",headerBg) hbCorner.CornerRadius=UDim.new(0,5)

    local headerLbl=Instance.new("TextLabel",headerBg)
    headerLbl.Size=UDim2.new(1,-8,1,0) headerLbl.Position=UDim2.new(0,6,0,0)
    headerLbl.BackgroundTransparency=1 headerLbl.Font=Enum.Font.GothamBold
    headerLbl.TextSize=10 headerLbl.TextColor3=WHITE
    headerLbl.TextXAlignment=Enum.TextXAlignment.Left
    headerLbl.Text=labelText:upper()

    -- Divider tipis
    local div=Instance.new("Frame",card)
    div.Size=UDim2.new(1,-12,0,1) div.Position=UDim2.new(0,6,0,20)
    div.BackgroundColor3=RED div.BackgroundTransparency=0.6 div.BorderSizePixel=0

    -- Label status di kiri bawah card
    local statusLbl=Instance.new("TextLabel",card)
    statusLbl.Size=UDim2.new(0.55,0,0,28) statusLbl.Position=UDim2.new(0,8,0,22)
    statusLbl.BackgroundTransparency=1 statusLbl.Font=Enum.Font.Gotham
    statusLbl.TextSize=10 statusLbl.TextColor3=DIM
    statusLbl.TextXAlignment=Enum.TextXAlignment.Left
    statusLbl.Text="Status: OFF"

    -- Toggle switch on/off di kanan bawah card
    local swBg=Instance.new("Frame",card)
    swBg.Size=UDim2.new(0,36,0,18) swBg.Position=UDim2.new(1,-44,0,26)
    swBg.BackgroundColor3=Color3.fromRGB(30,30,30) swBg.BorderSizePixel=0
    Instance.new("UICorner",swBg).CornerRadius=UDim.new(0.5,0)
    local swStroke=Instance.new("UIStroke",swBg) swStroke.Color=RED swStroke.Thickness=1

    local swKnob=Instance.new("Frame",swBg)
    swKnob.Size=UDim2.new(0,14,0,14) swKnob.Position=UDim2.new(0,2,0.5,-7)
    swKnob.BackgroundColor3=DIM swKnob.BorderSizePixel=0
    Instance.new("UICorner",swKnob).CornerRadius=UDim.new(0.5,0)

    -- Shared update function untuk checklist + card + switch
    local function upd()
        local on=State[key]
        -- checklist
        chk.Text = on and "✓" or ""
        ...
        -- card header tint
        headerBg.BackgroundTransparency = on and 0.2 or 0.4
        -- status text
        statusLbl.Text = on and "Status: ON" or "Status: OFF"
        statusLbl.TextColor3 = on and Color3.fromRGB(100,255,100) or DIM
        -- toggle switch
        swBg.BackgroundColor3 = on and Color3.fromRGB(35,8,8) or Color3.fromRGB(30,30,30)
        swKnob.BackgroundColor3 = on and RED or DIM
        swKnob.Position = on and UDim2.new(1,-16,0.5,-7) or UDim2.new(0,2,0.5,-7)
    end
    upd()

    -- Tombol transparan menutupi seluruh wrapper agar clickable di mana saja
...
local function setTab(name)
    ...
    elseif name=="Exploits" then
        ...
        toggle("Speed Fire (50x)","SpeedFire",9,function(v)
            applyFastFire(v)
        end)
    end
end
